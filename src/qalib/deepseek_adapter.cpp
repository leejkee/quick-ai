...
DeepSeekAdapter::DeepSeekAdapter(const std::string_view model,
    ...
    : m_api_key(api_key), m_model(model), m_url(url)
{
}


std::optional<CommonChatResponse>
DeepSeekAdapter::no_streaming_request(const std::vector<Message>& messages)
{
    if (messages.empty())
    {
        return std::nullopt;
    ...
    try
    {
        ...
        const auto [id, choices, model, usage] =
                response_json.get<NoStreamingResponsePacket>();
...
std::optional<CommonChatResponse>
DeepSeekAdapter::streaming_request(const std::vector<Message>& messages,
    ...
    session.SetWriteCallback(cpr::WriteCallback(
            [&](const std::string_view data, intptr_t userdata)
            {
                ...
                while ((pos = buffer.find("\n\n")) != std::string::npos)
                {
                    ...
                    {
                        std::string jsonData =
                                sse_chunk.substr(prefix.length());
                        // ...data: <L>{xxx}<R>...
                        ...
                        try
                        {
                            const auto unknow_json =
                                    nlohmann::json::parse(jsonData);

                            if (const auto choices = unknow_json["choices"][0];
                                choices.contains("finish_reason") &&
                                !choices["finish_reason"].is_null() &&
                                unknow_json.contains("usage"))
                            {
                                response.finish_reason =
                                        choices["finish_reason"];
                                response.usage =
                                        unknow_json["usage"].get<Usage>();
                                response.id = unknow_json["id"];
                                response.model_name = unknow_json["model"];
                            }
                            else if (choices["delta"].contains("content") &&
                                     !choices["delta"]["content"].is_null())
                            {
