...
void handle_mock_openai_streaming_completion(const httplib::Request& req,
                                               httplib::Response& res);

void handle_mock_openai_non_streaming_completion(const httplib::Request& req,
                                                   httplib::Response& res);
const auto MOCK_HANDLER =
        [](const httplib::Request& req, httplib::Response& res)
        {
            ...
            if (is_streaming)
            {
                ...
                handle_mock_openai_streaming_completion(req, res);
            }
            else
            {
                std::cout << "[Mock Server] Handling NO-STREAMING request."
                          << std::endl;
                handle_mock_openai_non_streaming_completion(req, res);
            }
        };
void handle_mock_openai_non_streaming_completion(const httplib::Request& req,
                                                   httplib::Response& res)
...
TEST_CASE("OpenAI Implement against Local Server", "[llm_protocol]")
{
    ...
    QA::Core::ModelMeta model_meta {MOCK_MODEL, MOCK_API_KEY, mock_url};
    auto client = QA::Core::LLMClientFactory::create_llm_client(model_meta);
    std::vector<QA::Core::Message> messages = {{"user", "Hi"}};

    SECTION("No streaming request Test")
    {
        auto response = client->no_streaming_chat(QA::Core::ModelParams(), messages);
        REQUIRE(response.has_value());
        REQUIRE(response->message.role == "assistant");
        REQUIRE(response->message.content == MOCK_RESPONSE_CONTENT);
        REQUIRE(response->total_tokens == 21);
    }
